...
interface MudancaFormData {
  ...
  aceitouServico: string;
  autorizacaoExcecao: string;
  nomeAutorizador: string;
  horarioEspecifico: string;
}

const initialFormState: MudancaFormData = {
  solicitacao: 'MUDANÇA DE ENDEREÇO',
  taxa: 'ISENTO',
  podeAdiantar: 'Não',
  dataAgendamento: '',
  periodo: '',
...
interface MudancaEnderecoProps {
  operatorName: string;
}

export function MudancaEndereco({ operatorName }: MudancaEnderecoProps) {
  const [formData, setFormData] = useState<MudancaFormData>(initialFormState);
  ...
  const validateForm = () => {
    ...
    const requiredFields = [
      'dataAgendamento', 'periodo', 'clienteDesde', 'telefone', 
      'enderecoAtual', 'novoEndereco', 'comprovanteEndereco', 'aceitouServico'
    ];
    
    requiredFields.forEach(field => {
      if (!formData[field as keyof MudancaFormData]) {
...
Maps: ${formData.localizacaoMaps}

=== VERIFICAÇÕES ===
Comprovante: ${formData.comprovanteEndereco}
Ponto Adicional: ${formData.pontoAdicional}
Aceitou Serviço: ${formData.aceitouServico}
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <h1 className="text-2xl font-bold text-slate-800">Mudança de Endereço</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Agendamento & Detalhes */}
        <Card icon={<Calendar className="w-4 h-4" />} title="Agendamento & Detalhes" className="lg:col-span-3">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
              <label className="block text-xs font-medium text-slate-500 mb-1">Solicitação (Fixo)</label>
              <input 
                type="text" 
                value={formData.solicitacao} 
                readOnly 
                className="w-full p-2 border border-slate-200 rounded-lg bg-slate-50 text-slate-500 font-bold"
              />
            </div>
            <div>
              <label className="block text-xs font-medium text-slate-500 mb-1">Taxa (Fixo)</label>
              <input 
                type="text" 
                value={formData.taxa} 
                readOnly 
            ...
            <div>
              <label className="block text-xs font-medium text-slate-500 mb-1">Pode adiantar?</label>
                ...
                {['Sim', 'Não'].map(option => (
                  <label key={option} className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-lg border text-sm font-medium transition-all cursor-pointer ${
                    ...
                    <input 
                      ...
                      name="podeAdiantar" 
                      value={option} 
                      checked={formData.podeAdiantar === option}
        ...
        {/* Endereços e Contato */}
        <Card icon={<MapPin className="w-4 h-4" />} title="Endereços e Contato" className="lg:col-span-2">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="md:col-span-2">
              ...
              <input 
                ...
                value={formData.enderecoAtual}
                onChange={handleInputChange}
                className={getFieldClass('enderecoAtual')}
              />
            </div>
            <div className="md:col-span-2 bg-red-50/50 p-4 rounded-xl border border-red-100">
              <label className="block text-xs font-bold text-red-600 mb-1 uppercase tracking-wider">Novo Endereço</label>
              <input 
                type="text" 
                name="novoEndereco"
                placeholder="Rua, Número, Bairro - Cidade"
                ...
                onChange={handleInputChange}
                className={getFieldClass('novoEndereco', 'bg-white')}
              />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:col-span-2">
              <div>
                <label className="block text-xs font-medium text-slate-500 mb-1">Referência</label>
                <input 
                  type="text" 
                  name="referencia"
              ...
              <div>
                <label className="block text-xs font-medium text-slate-500 mb-1">Localização Maps (Link)</label>
                <input 
                  type="text" 
                  name="localizacaoMaps"
                  value={formData.localizacaoMaps}
                  onChange={handleInputChange}
                  className={getFieldClass('localizacaoMaps')}
                />
              </div>
            </div>
          </div>
        ...
        {/* Verificações & ETECC Resolve */}
        <div className="space-y-6">
          <Card icon={<ClipboardList className="w-4 h-4" />} title="Verificações">
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-medium text-slate-500 mb-1">Comprovante de Endereço</label>
                <select 
                  name="comprovanteEndereco"
                  value={formData.comprovanteEndereco}
                  onChange={handleInputChange}
                  className={getFieldClass('comprovanteEndereco', 'bg-white')}
                >
                  <option value="">Selecione...</option>
              ...
              <div>
                <label className="block text-xs font-medium text-slate-500 mb-1">Cliente tem ponto adicional?</label>
                <div className="flex gap-2">
                  {['Sim', 'Não'].map(option => (
                    <label key={option} className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-lg border text-sm font-medium transition-all cursor-pointer ${
                      formData.pontoAdicional === option 
                        ? 'bg-red-50 border-red-200 text-red-600' 
                        : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50'
                    }`}>
                      <input 
                        ...
                        name="pontoAdicional" 
                        value={option} 
                        checked={formData.pontoAdicional === option}
                        onChange={handleInputChange}
                        className="w-4 h-4 text-red-600 focus:ring-red-500"
                      />
                      {option}
                ...
                {formData.pontoAdicional === 'Sim' && (
                  <div className="mt-3 p-3 bg-amber-50 border border-amber-200 rounded-lg flex items-start gap-2">
                    ...
                    <p className="text-[11px] text-amber-800 leading-tight">
                      <span className="font-bold">Atenção:</span> Notifique o cliente sobre avaliação desse ponto para a nova residência.
                    </p>
                  </div>
                )}
              </div>
          ...
          <Card icon={<CheckCircle2 className="w-4 h-4" />} title="ETECC Resolve">
            <div>
              <label className="block text-xs font-medium text-slate-500 mb-1">Cliente aceitou serviço de mudança?</label>
              <select 
                name="aceitouServico"
                value={formData.aceitouServico}
                onChange={handleInputChange}
                className={getFieldClass('aceitouServico', 'bg-white')}
              >
                <option value="">Selecione...</option>
        ...
        <Card icon={<ShieldCheck className="w-4 h-4" />} title="Autorização & Atendente" className="lg:col-span-3">
